...
JoinAtBack::JoinAtBack(GeneralPlatooningApp *app) :
        JoinManeuver(app), joinManeuverState(JoinManeuverState::IDLE) {
}
...
void JoinAtBack::startManeuver(const void *parameters) {
    if (initializeJoinManeuver(parameters)) {
        ...
        LOG << positionHelper->getId()
                   << " sending JoinPlatoonRequesto to platoon with id "
...
bool JoinAtBack::processJoinRequest(const JoinPlatoonRequest *msg) {
    ...
    LOG << positionHelper->getId()
               << " sending JoinPlatoonResponse to vehicle with id "
...
void JoinAtBack::handleJoinPlatoonResponse(const JoinPlatoonResponse *msg) {
    ...
    if (msg->getPermitted()) {
        LOG << positionHelper->getId()
                   << " received JoinPlatoonResponse (allowed to join)\n";
        ...
        LOG << positionHelper->getId()
                   << " received JoinPlatoonResponse (not allowed to join)\n";
        // abort maneuver
